    ...
    if (count($_POST) > 0) {
        ...
        if ($id == 0) {
            ...
            if (!$dup) {
                ...
                if ($que == "OK") {
                    $message = "Project Category has been Created Successfully.";
                    $cl = "success";
                    $fa = "fa-check";
                } else {
            ...
            if ($que == 1) {
                $message = "Project Category has been Updated Successfully.";
                $cl = "success";
                $fa = "fa-check";
            } else {
...
{
    try {
        $query = "SELECT `category` FROM `project_category` WHERE `category`='$name'";
...
{
    try {
        $query = "INSERT INTO `project_category` SET `category`='$name',`description`='$description'";
        $que = $db->insert($query);
        return $que;
    } catch (Exception $e) {
        return $e->getMessage();
...
function update($db, $id, $name, $description)
{
    $query = "";
    try {
        $query = "UPDATE `project_category` SET `category`='$name',`description`='$description' WHERE `id` = $id";
        $que = $db->query($query);
        if($que){
            return 1;
        }else{
...
{
    try {
        $query = "DELETE FROM `project_category`  WHERE `id` = $id";
        $que = $db->query($query);
        return $que;
    } catch (Exception $e) {
        return $e->getMessage();
