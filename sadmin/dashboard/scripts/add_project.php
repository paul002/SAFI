...
if (isset($_GET['projectId'])) {
    ...
    if (count($_POST) > 0) {
        ...
        // Form validation
        if ($name == null || $start_date == "" || $end_date == "" || $category == "" || $sponsor == "") {
        ...
        if ($featuredImage['name'] != null && $featuredImage['error'] != 4) {
            $path = $_SERVER['DOCUMENT_ROOT'] . "/safimw.org/assets/images/projects/";
        ...
        if ($id == 0) {
            ...
            if (!$dup) {
                //	SAVE RECORD                                                
                $que = save($dbo, $name, $content_description, $sDate, $eDate, $category, $sponsor, $fileName);
                if ($que == "OK") {
...
function save($db, $name, $description, $start_date, $end_date, $category, $sponsor, $featImg)
{
    try {
        $query = "INSERT INTO `projects` SET `name`='$name',`description`='$description',`start_date`='$start_date',`end_date`='$end_date',`category`='$category',`sponsor`='$sponsor', `featured_image`='$featImg'";
        $que = $db->insert($query);
        return $que;
    } catch (Exception $e) {
        return $e->getMessage();
...
function update($db, $id, $name, $description, $start_date, $end_date, $category, $sponsor, $featImg)
{
    $query = "";
    try {
        if ($featImg != "") {
            $query = "UPDATE `projects` SET `name`='$name',`description`='$description',`start_date`='$start_date',`end_date`='$end_date',`category`='$category',`sponsor`='$sponsor', `featured_image`='$featImg' WHERE `id` = $id";
        } else {
            $query = "UPDATE `projects` SET `name`='$name',`description`='$description',`start_date`='$start_date',`end_date`='$end_date',`category`='$category',`sponsor`='$sponsor', `featured_image`='$featImg' WHERE `id` = $id";
        }
        $que = $db->insert($query);
...
function delete($db, $id)
{
    try {
        ...
        $fQuery = "SELECT * FROM `projects` WHERE `id` = $id";
        ...
        if ($featImg != "") {
            if (!unlink($targetFile)) {
                ...
                $pQuery = "DELETE FROM `projects` WHERE `id` = $id";
                $que = $db->query($pQuery);
                return $que;
            }
        } else {
